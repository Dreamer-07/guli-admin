...
export default {
  ...
  data() {
    return {
      ...
      courseId: this.$route.params.courseId, // 所属课程
      coursePublish: {}
    }
  },
  mounted() {
  ...
  methods: {
    ...
    async publish() {
      await courseApi.updateCoursePublishInfo(this.courseId);
      this.$message.success('发布课程成功')
      this.$router.push({name: 'CourseIndex'})
    },
    async getCoursePublishInfo() {
      const {data} = await courseApi.getCoursePublishInfo(this.courseId)
      this.coursePublish = data
    }
  }
}
</script>
